...
CREATE TABLE IF NOT EXISTS public.hotels (
  ...
  secondary_color TEXT NOT NULL DEFAULT '#f4f7f5',
  phone TEXT,
  email TEXT,
  address TEXT,
...
CREATE OR REPLACE FUNCTION public.current_hotel_role()
RETURNS TEXT
LANGUAGE SQL
STABLE
SECURITY DEFINER
...
CREATE POLICY hotel_member_select ON public.hotels FOR SELECT TO authenticated
USING (id = public.current_hotel_id());

DROP POLICY IF EXISTS hotel_member_update ON public.hotels;
CREATE POLICY hotel_member_update ON public.hotels FOR UPDATE TO authenticated
USING (id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'))
WITH CHECK (id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'));

DROP POLICY IF EXISTS hotel_membership_select ON public.hotel_memberships;
CREATE POLICY hotel_membership_select ON public.hotel_memberships FOR SELECT TO authenticated
USING (hotel_id = public.current_hotel_id());

DROP POLICY IF EXISTS hotel_membership_manager_insert ON public.hotel_memberships;
CREATE POLICY hotel_membership_manager_insert ON public.hotel_memberships FOR INSERT TO authenticated
...
CREATE POLICY hotel_membership_manager_update ON public.hotel_memberships FOR UPDATE TO authenticated
...
-- Create the first/default hotel for the existing single-tenant installation.
DO $$
DECLARE
  owner_id UUID;
  hotel_id UUID;
BEGIN
  ...
  IF owner_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.hotels) THEN
    INSERT INTO public.hotels (name, slug, tagline, created_by)
    VALUES ('TillBook Hotel', 'tillbook-hotel', 'Hotel & restaurant operations', owner_id)
    RETURNING id INTO hotel_id;

    INSERT INTO public.hotel_memberships (hotel_id, user_id, role)
    SELECT hotel_id, ur.user_id,
      CASE WHEN ur.role::TEXT = 'manager' THEN 'manager' ELSE 'cashier' END
    FROM public.user_roles ur
    ON CONFLICT (hotel_id, user_id) DO NOTHING;

    UPDATE public.hotel_memberships
    SET role = 'owner'
    WHERE hotel_id = hotel_id AND user_id = owner_id;
  END IF;
END $$;

-- Add a tenant key to every business table. Existing rows are assigned to the
-- migrated default hotel. Inserts from the application can omit hotel_id;
-- the trigger below supplies the authenticated user's active hotel.
DO $$
DECLARE
  table_name TEXT;
  tables TEXT[] := ARRAY[
    'menu_items','orders','order_items','inventory_items','stock_movements',
    'recipes','recipe_ingredients','staff_shifts'
  ];
BEGIN
  FOREACH table_name IN ARRAY tables LOOP
    ...
    EXECUTE format($f$
      UPDATE public.%I t
      SET hotel_id = h.id
      FROM (SELECT id FROM public.hotels ORDER BY created_at LIMIT 1) h
      WHERE t.hotel_id IS NULL
    $f$, table_name);
  END LOOP;
END $$;

CREATE OR REPLACE FUNCTION public.set_business_hotel_id()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF active_hotel IS NULL THEN
    RAISE EXCEPTION 'No active hotel account is associated with this user';
  END IF;
  IF NEW.hotel_id IS NULL THEN
    NEW.hotel_id := active_hotel;
  ELSIF NEW.hotel_id <> active_hotel THEN
    RAISE EXCEPTION 'Cross-hotel data access is not allowed';
  END IF;
  RETURN NEW;
END;
$$;
...
-- Tenant-aware RLS for business data. Existing policies are removed so no
-- previous single-tenant policy can accidentally expose another hotel's rows.
DO $$
DECLARE
  policy_row RECORD;
  tables TEXT[] := ARRAY[
...
-- General authenticated access is retained for the POS, while manager-only
-- mutations remain enforced in the application where appropriate.
CREATE POLICY tenant_menu_select ON public.menu_items FOR SELECT TO authenticated
USING (hotel_id = public.current_hotel_id());
CREATE POLICY tenant_menu_insert ON public.menu_items FOR INSERT TO authenticated
WITH CHECK (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'));
CREATE POLICY tenant_menu_update ON public.menu_items FOR UPDATE TO authenticated
USING (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'))
WITH CHECK (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'));
CREATE POLICY tenant_menu_delete ON public.menu_items FOR DELETE TO authenticated
USING (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'));
...
CREATE POLICY tenant_inventory_all ON public.inventory_items FOR ALL TO authenticated
USING (hotel_id = public.current_hotel_id()) WITH CHECK (hotel_id = public.current_hotel_id());
CREATE POLICY tenant_stock_all ON public.stock_movements FOR ALL TO authenticated
USING (hotel_id = public.current_hotel_id()) WITH CHECK (hotel_id = public.current_hotel_id());

CREATE POLICY tenant_recipes_all ON public.recipes FOR ALL TO authenticated
USING (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'))
WITH CHECK (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'));
CREATE POLICY tenant_recipe_ingredients_all ON public.recipe_ingredients FOR ALL TO authenticated
USING (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'))
WITH CHECK (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'));

CREATE POLICY tenant_shifts_select ON public.staff_shifts FOR SELECT TO authenticated
USING (hotel_id = public.current_hotel_id());
CREATE POLICY tenant_shifts_insert ON public.staff_shifts FOR INSERT TO authenticated
WITH CHECK (hotel_id = public.current_hotel_id() AND user_id = auth.uid());
CREATE POLICY tenant_shifts_update ON public.staff_shifts FOR UPDATE TO authenticated
USING (hotel_id = public.current_hotel_id() AND (user_id = auth.uid() OR public.current_hotel_role() IN ('owner','manager')))
WITH CHECK (hotel_id = public.current_hotel_id());
CREATE POLICY tenant_shifts_delete ON public.staff_shifts FOR DELETE TO authenticated
USING (hotel_id = public.current_hotel_id() AND public.current_hotel_role() IN ('owner','manager'));

GRANT SELECT, INSERT, UPDATE, DELETE ON public.menu_items, public.orders, public.order_items,
  public.inventory_items, public.stock_movements, public.recipes, public.recipe_ingredients,
  public.staff_shifts TO authenticated;

-- Existing role helper now resolves the active hotel when a tenant-aware role
-- exists, while retaining compatibility with older user_roles callers.
...
-- First-user onboarding RPC. It atomically creates the hotel and makes the
-- signing-up user its owner/manager.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.hotel_memberships WHERE user_id = auth.uid() AND status = 'active') THEN
    RAISE EXCEPTION 'This account already belongs to a hotel';
  END IF;
  ...
  INSERT INTO public.hotels (name, slug, tagline, logo_url, primary_color, secondary_color, phone, email, address, created_by)
  VALUES (trim(hotel_name), hotel_slug, NULLIF(trim(hotel_tagline), ''), NULLIF(trim(hotel_logo_url), ''),
          hotel_primary_color, hotel_secondary_color, NULLIF(trim(hotel_phone), ''), NULLIF(trim(hotel_email), ''),
          NULLIF(trim(hotel_address), ''), auth.uid())
  RETURNING id INTO new_hotel_id;

  INSERT INTO public.hotel_memberships (hotel_id, user_id, role)
  VALUES (new_hotel_id, auth.uid(), 'owner');
  ...
  SET approval_status = 'approved', job_title = 'Owner'
  WHERE id = auth.uid();
...
BEGIN
  ...
  SET name = trim(hotel_name), tagline = NULLIF(trim(hotel_tagline), ''), logo_url = NULLIF(trim(hotel_logo_url), ''),
      primary_color = hotel_primary_color, secondary_color = hotel_secondary_color,
      phone = NULLIF(trim(hotel_phone), ''), email = NULLIF(trim(hotel_email), ''), address = NULLIF(trim(hotel_address), ''),
      updated_at = now()
