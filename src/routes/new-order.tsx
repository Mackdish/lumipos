      ...
      <main className="mx-auto max-w-7xl px-4 pb-28 pt-5 sm:px-8 sm:py-8 lg:px-10 lg:pb-10">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
          <section className="min-w-0">
            ...
            {isLoading && <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{[1,2,3,4,5,6].map((n) => <div key={n} className="h-52 animate-pulse rounded-2xl bg-muted" />)}</div>}
          ...
          <aside className="h-fit rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5 lg:sticky lg:top-6">
            ...
            <div className="mt-4 border-t border-border pt-4">{items.length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">Select a product card to start.</p> : <ul className="space-y-3">{items.map((i) => <li key={i.name} className="flex items-center justify-between gap-3 text-sm"><span className="min-w-0 truncate"><b>{i.quantity}×</b> {i.name}</span><div className="flex shrink-0 items-center gap-2"><button type="button" onClick={() => { const menuItem = menu.find((m) => m.name === i.name); if (menuItem) bump(menuItem.id, -1); }} className="grid h-7 w-7 place-items-center rounded-lg border border-border font-bold">−</button><span className="w-4 text-center text-xs font-bold">{i.quantity}</span><button type="button" onClick={() => { const menuItem = menu.find((m) => m.name === i.name); if (menuItem) bump(menuItem.id, 1); }} className="grid h-7 w-7 place-items-center rounded-lg bg-primary text-xs font-bold text-primary-foreground">+</button></div></li>)}</ul>}</div>
