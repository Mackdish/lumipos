...
function OrderDetail() {
  ...
  async function update(patch: { payment_status?: string; order_status?: string; approved_by?: string | null; approved_at?: string | null }) { const { error } = await supabase.from("orders").update(patch).eq("id", id); if (error) { toast.error(error.message); return; } await queryClient.invalidateQueries({ queryKey: ["order", id] }); await queryClient.invalidateQueries({ queryKey: ["orders"] }); toast.success("Order updated"); }
  ...
  return <AppShell><main className="mx-auto max-w-3xl px-4 pb-28 pt-5 sm:px-8 sm:py-8 lg:px-10 lg:pb-10">
    ...
    {isLoading ? <div className="mt-5 space-y-3"><div className="h-10 animate-pulse rounded-xl bg-muted" /><div className="h-48 animate-pulse rounded-2xl bg-muted" /></div> : isError ? <div className="mt-5 rounded-2xl border border-destructive/20 bg-destructive/5 p-5 text-sm text-destructive">Unable to load this order.</div> : !order ? <div className="mt-5 rounded-2xl border border-border bg-card p-8 text-center"><p className="font-semibold">Order not found</p><Link to="/orders" className="mt-3 inline-flex text-sm font-bold text-primary">Return to orders</Link></div> : <>
      ...
      {order.payment_status !== "PAID" && <section className="mt-4 rounded-2xl border border-border bg-card p-4 shadow-sm"><div><p className="font-bold">Payment status</p><p className="mt-1 text-sm text-muted-foreground">Review the order before moving it forward.</p></div><div className="mt-4 flex flex-col gap-3 sm:flex-row">
        <button type="button" onClick={() => update({ payment_status: "PAID", approved_by: user?.id ?? null, approved_at: new Date().toISOString() })} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground sm:flex-1">Approve order</button>
      </div></section>}
    </>}
  </main></AppShell>;
}
