    ...
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground lg:grid lg:grid-cols-[244px_minmax(0,1fr)]">
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-border bg-card/95 px-4 backdrop-blur lg:hidden">
        <Link to="/" className="flex min-w-0 items-center gap-2.5">
          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-primary text-xs font-black text-primary-foreground">LP</span>
          <span className="min-w-0"><b className="block truncate text-sm">LumiPOS</b><span className="block text-[10px] text-muted-foreground">Restaurant operations</span></span>
        </Link>
      ...
      <aside className="hidden min-h-screen border-r border-border bg-card p-4 lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col">
        <Link to="/" className="mb-10 flex items-center gap-3 px-2 pt-2">
          <span className="grid h-10 w-10 place-items-center rounded-xl bg-primary text-sm font-black text-primary-foreground">LP</span>
          <span><b className="block text-sm">LumiPOS</b><span className="block text-xs text-muted-foreground">Restaurant operations</span></span>
        </Link>
      ...
      <nav aria-label="Mobile navigation" className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 px-2 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_18px_rgba(0,0,0,0.06)] backdrop-blur lg:hidden">
        <div className="mx-auto grid h-16 max-w-md grid-cols-5">
          ...
          <button type="button" onClick={signOut} className="flex min-w-0 flex-col items-center justify-center gap-0.5 rounded-xl text-[10px] font-bold text-destructive transition hover:bg-destructive/10">
            <span aria-hidden="true" className="grid h-7 w-10 place-items-center rounded-xl text-base">↪</span>
